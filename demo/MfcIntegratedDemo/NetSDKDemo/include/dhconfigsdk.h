...
#define MAX_VIDEODIAGNOSIS_DETECT_TYPE          64          // Video diagnosis type max number
...
#define CFG_CAP_CMD_VIDEOANALYSE                "devVideoAnalyse.getCaps"					// Video analyze capability(CFG_CAP_ANALYSE_INFO, pExtendInfo corresponding of CFG_CAP_ANALYSE_REQ_EXTEND_INFO)
#define CFG_CAP_CMD_VIDEOANALYSE_EX             "devVideoAnalyse.getCapsEx"	                // Video analyze capability(CFG_CAP_ANALYSE_INFO_EX)
#define CFG_CAP_CMD_REMOTE_VIDEOANALYSE         "devRemoteVideoAnalyse.getCaps"             // Remote Video analyze capability(CFG_CAP_ANALYSE_INFO)
...
#define CFG_CAP_CMD_REMOTE_EVENTMANAGER         "RemoteEventManager.getCaps"                // The Ability to obtain remote equipment alarm linkage (CFG_CAP_EVENTMANAGER_INFO)
...
typedef struct tagCFG_AUDIO_PLAY_PATH
{
    ...
    BOOL           bSupportUpload;         // if support upload
    int            nMaxFileUploadNum;      // max file upload number, valid when support upload
    int            nMaxUploadFileSize;     // max upload file size, valid when support upload
} CFG_AUDIO_PLAY_PATH;
...
typedef struct CFG_CAP_SPEAK
{
    ...
    int                     nAudioPlayPathNum;                 // audio play path number
    CFG_AUDIO_PLAY_PATH     stuAudioPlayPath[8];               // audio play path
}CFG_CAP_SPEAK;
...
typedef enum tagEM_SCENE_TYPE
{
    ...
    EM_SCENE_INTELLI_FINANCE,   // "IntelliFinance"
	EM_SCENE_CROWD_ABNORMAL,	// "CrowdAbnormal"
	EM_SCENE_ANATOMYTEMP_DETECT,	// "AnatomyTempDetect"
	EM_SCENE_WEATHER_MONITOR,		// "WeatherMonitor"
	EM_SCENE_ELEVATOR_ACCESS_CONTROL,// "ElevatorAccessControl"
}EM_SCENE_TYPE;
...
// Real-time frame drawing config, EVS customization
typedef struct tagCFG_BACKUP_LIVE_INFO
{   
    BOOL                bEnable;                                        // whether to enable, start frame drawing
    int                 nBackupRate;                                    // If it is 0, only I frame will be reserved. In other cases, I frame and several P frames next to it will be reserved
                                                                        // Unit: percentage
    ...
    CFG_TIME_SECTION    stuTimeSection;                                 // frame drawing time section
...
typedef struct tagCFG_RECORD_INFO
{
    ...
    int                      nBackupLiveNum;                // Real-time frame drawing config number
    CFG_BACKUP_LIVE_INFO     stuBackupLiveInfo[8];          // Real-time c config, EVS customization
} CFG_RECORD_INFO;
...
typedef struct tagCFG_ALARM_MSG_HANDLE
{
	...
	bool				abAlarmOutTime;								// is nAlarmOutTime valid
	int					nAlarmOutTime;								// alarm timeout,unit:second(customized)
	bool				abBeepTime;									// is nBeepTime valid
	int					nBeepTime;									// beep time(customized),unit:second,max value is 3600,0 means beep continuely
...
{
    int                 nChannel;                      // link channel id
    EM_SCENE_TYPE       emClassType;                   // link class type
    BOOL                bSupportAllTimeWork;           // if support all time work
    BYTE                byReserved[252];               // reserved
} CFG_LINK_CLASS_TYPE;

// Active analyse capability
typedef struct tagCFG_ACTIVITY_ANALYSE_CAPS
{
	BOOL				bSupportLocalDataStore;	// Whether support local data store
	UINT				nMaxRules;				// The maximum rules of that capability
...
typedef struct
{
    ...
    BOOL                    bSupportPolygon;                                            // Whether the CrowdDistriMap supports polygons, TRUE: supports any edge shape; FALSE: does not have the ability to represent or only supports 4-point parallelogram
	CFG_ACTIVITY_ANALYSE_CAPS	stuActivityAnalyseCaps;								// Active analyse capability
    int                          nLink;                                             // link number
    CFG_LINK_CLASS_TYPE          stuLinkClassType[16];                              // link class type
}SCENE_SUPPORT_RULE;
...
// Ability of calibration line  
typedef struct tagCROWD_CALIBRATION
{
    UINT    nHorizontalLines;       // Number of horizontal lines
    UINT    nVerticalLines;         // Number of vertical lines
}CROWD_CALIBRATION;

// Intelligent type
typedef enum tagCFG_EM_FACEDETECT_TYPE
{
	CFG_EM_FACEDETECT_TYPE_UNKNOWN = 0,										// Unknown
	CFG_EM_FACEDETECT_TYPE_VISUAL,											// Visual-light
	CFG_EM_FACEDETECT_TYPE_THERMAL,											// Infrared ray
	CFG_EM_FACEDETECT_TYPE_SWITCHBYTIME,									// Switch by time
}CFG_EM_FACEDETECT_TYPE;

// The capacity of measure body temperature
typedef struct tagCFG_ANATOMYTEMP_VISUAL_CAPS
{
	BOOL							bSupportFaceRight;							// whether support face left and right angles filtering
	BOOL							bSupportFaceUp;								// whether support face up and down angles filtering
	BOOL							bSupportFaceRoll;							// whether support face turned left and right angles filtering
...
{
	CFG_EM_FACEDETECT_TYPE	emFaceDetectTypes[32];								// The intelligent types of face detection supported 
	int					nFaceDetectTypesNum;									// The number of intelligent types in szFaceDetectTypes
	CFG_ANATOMYTEMP_VISUAL_CAPS	stuVisual;										// The capacity of visual-light
	BYTE				byReserved[1024];										// Reserved
}CFG_ANATOMYTEMP_DETECT_SCENE_CAPS;

// The scene of stereo behavior 
typedef struct tagCFG_STEREO_BEHAVIOR_SCENE_CAPS
{
	int					nEventListNum;											// The number of events in the list
	DWORD				dwOnlyFindLastEventList[32];							// Which event support find last event information
...
typedef struct tagCFG_CAP_SCENE
{
    ...
    CROWD_CALIBRATION stuCalibration;                                           // Ability of calibration line 
	CFG_ANATOMYTEMP_DETECT_SCENE_CAPS	stuAnatomyTempDetect;					// The scene of measure body temperature
	CFG_STEREO_BEHAVIOR_SCENE_CAPS	stuStereoBehaviorCaps;						// The scene of stereo behavior 
	BYTE              byReserved[1024];                                         // Reserved
}CFG_CAP_SCENE;
...
{
    EM_SCENE_TYPE       emSceneType[32];    // Supported scene types    
    int					nSceneNum;			// Supported scene num
    BYTE				byReserved[252];	// Reserved
...
typedef struct tagCFG_CAP_ANALYSE_INFO
{
    ...
    VA_DYNAMIC_CAPS_INFO stuDynamicCaps;                                        // Dynamic capability set, which represents the capability of the current video channel
} CFG_CAP_ANALYSE_INFO;

// subclass type
typedef enum tagCFG_EM_INSTANCE_SUBCLASS_TYPE
{
	CFG_EM_INSTANCE_SUBCLASS_TYPE_LOCAL = 0,		// local
	CFG_EM_INSTANCE_SUBCLASS_TYPE_REMOTE,			// remote
}CFG_EM_INSTANCE_SUBCLASS_TYPE;

// The extend information of video analyse caps
typedef struct tagCFG_CAP_ANALYSE_REQ_EXTEND_INFO
{
	DWORD							dwSize;			// struct size
	CFG_EM_INSTANCE_SUBCLASS_TYPE	emSubClassID;	// subclass type
...
// Visual-light information
typedef struct tagCFG_FACEDETECT_VISUAL_INFO
{
	UINT						nFaceAngleUp;								// The maximum up or down offset angle of faces to be detected ,Unit:degree,range[0-90].
	UINT						nFaceAngleRight;							// The maximum left or right offset angle of faces to be detected ,Unit:degree,range[0-90].
	UINT						nFaceRollRight;								// The maximum left or right crooked offset angle of faces to be detected ,Unit:degree,range[0-90].
	BYTE						byReserved[1024];							// Reserved
...
{
	CFG_EM_FACEDETECT_TYPE			emFaceDetectType;								// The intelligent type of face detection
	CFG_FACEDETECT_VISUAL_INFO		stuVisual;										// The visual light information, enable when emFaceDetectType is CFG_EM_FACEDETECT_TYPE_VISUAL or CFG_EM_FACEDETECT_TYPE_TIMESECTION
	CFG_TIME_SECTION				stuTimeSection;									// The visual light time section, enable when emFaceDetectType is CFG_EM_FACEDETECT_TYPE_TIMESECTION
	BYTE							byReserved[1024];								// Reserved
...
typedef struct tagCFG_ANALYSEGLOBAL_SCENE
{
	...
	union
	{
		...
		CFG_ANATOMYTEMP_SCENCE_INFO		stuAnatomyTempScene;	// The scene of measure body temperature
	};
	...
	BOOL						bSceneTypeListEx;						// whether szSceneTypeListEx is valid: TRUE, use szSceneTypeListEx
	int							nSceneListCountEx;						// the valid num of szSceneTypeListEx
	char						szSceneTypeListEx[32][128];				// scene type extend, extend of szSceneTypeList
...
typedef struct tagCFG_ANALYSEGLOBAL_INFO
{
	...
	UINT				unLatitude;								    // dimension company:one in a million
	BOOL							bSceneTypeListEx;						// whether szSceneTypeListEx is valid: TRUE, use szSceneTypeListEx
	int								nSceneCountEx;							// the valid num of szSceneTypeListEx
	char							szSceneTypeListEx[32][128];				// extend of szSceneTypeList																			
	CFG_ANATOMYTEMP_SCENCE_INFO		stuAnatomyTempScene;			// The scene of measure body temperature
...
//szAttributes properties work when stored in "Category"
	...
	BOOL						bSceneTypeEx;								// whether szSceneTypeEx is valid: TRUE, use szSceneTypeListEx
	char						szSceneTypeEx[128];							// extend of szSceneType
} CFG_MODULE_INFO;
...
typedef struct tagCFG_CROSSLINE_INFO
{
    ...
    CFG_ALARM_MSG_HANDLE    stuRemoteEventHandler;                                        // Remote alarm event handler
    CFG_TIME_SECTION        stuRemoteTimeSection[WEEK_DAY_NUM][MAX_REC_TSECT_EX];         // Remote event respond time section
    BOOL                    bDisableRemoteTimeSection;                                    // Remote stuTimeSection ,default FALSE:not disable,TRUE:disable,user control
} CFG_CROSSLINE_INFO;
...
{
    EM_CFG_DETECTION_SCENE_TYPE_UNKNOWN,                                           // unknown
    EM_CFG_DETECTION_SCENE_TYPE_ANIMAL,                                            // Animal scene, which can detect animals, people and vehicles
    EM_CFG_DETECTION_SCENE_TYPE_BIRD,                                              // Bird scene, which can detect birds, people and vehicles
...
typedef struct tagCFG_ANIMAL_DETECTION_INFO
{
    ...
    EM_CFG_DETECTION_SCENE_TYPE    emDetectionSceneType;                        // Scene types
    BYTE				byReserved[1020]; 					                    // Reserved
}CFG_ANIMAL_DETECTION_INFO;

// Configuration of human body temperature measurement rules
typedef struct tagCFG_ANATOMY_TEMP_DETECT_INFO
{   
	char				szRuleName[MAX_NAME_LEN];								// Rule name. Different rules cannot have the same name
	BOOL				bRuleEnable;											// Rule enable
	int					nObjectTypeNum;											// Number of corresponding object types
	char				szObjectTypes[MAX_OBJECT_LIST_SIZE][MAX_NAME_LEN];		// List of corresponding object types
	int                 nPtzPresetId;                                           // Preset point no. of PTZ 0 ~ 65535
	CFG_ALARM_MSG_HANDLE stuEventHandler;										// Alarm linkage
	CFG_TIME_SECTION	stuTimeSection[WEEK_DAY_NUM][MAX_REC_TSECT_EX];			// Event response time period
	BYTE                bTrackEnable;                                           // Trigger tracking enable, only valid for cordon events, cordon rules
	int								nDetectRegionPoint;							// Polygon vertex
	CFG_POLYGON						stuDetectRegion[MAX_POLYGON_NUM];			// Detection area, polygon
	BOOL							bHighEnable;								// Whether the high temperature abnormal alarm is on
	BOOL							bLowEnable;									// Whether the low temperature abnormal alarm is on
	int								fHighThresholdTemp;							// High temperature abnormal threshold, magnify × 10
	int								fLowThresholdTemp;							// Low temperature abnormal threshold, magnify × 10
	BOOL							bIsAutoStudy;								// Automatic learning or not
	int								fHighAutoOffset;							// Deviation value of high temperature automatic learning, magnify × 10
	int								fLowAutoOffset;								// Deviation value of low temperature automatic learning,, magnify × 10
	int								nSensitivity;								// Sensitivity range [1, 10]
	BOOL							bSizeFileter;								// Is the rule specific size filter valid
	CFG_SIZEFILTER_INFO				stuSizeFileter;								// Rule specific size filters
	BOOL							bIsCaptureNormal;							// Whether to report normal temperature information
...
// Rule type EVENT_IVS_SLEEP_DETECT(Sleep detect) configuration
typedef struct tagCFG_SLEEP_DETECT_INFO
{
    ...
    UINT                    nMinDuration;                                           // Minimum duration in seconds, range (30s-300s)
    UINT                    nReportInterval;                                        // Repeat alarm interval, unit: second, range (0s-300s)
    UINT                    nDetectRegionPoint;                                     // The points number of region
    CFG_POLYGON             stuDetectRegion[MAX_POLYGON_NUM];                       // Detect region, point range[0,8192)
    BYTE                    byReserved[4092];                                       // Reversed
}CFG_SLEEP_DETECT_INFO;

// Rule type EVENT_IVS_WALK_AROUND_DETECT(Walk around detect) configuration
typedef struct tagDEV_CFG_WALK_AROUND_DETECT_INFO
{
    ...
    UINT                    nMinDuration;                                           // Minimum duration in seconds, range (10S?0S)
    UINT                    nReportInterval;                                        // Repeat alarm interval, unit: second, range (0s-300s)
    UINT                    nSensitivity;                                           // Sensitivity, the smaller the value, the lower the sensitivity. Value 1-10
    UINT                    nDetectRegionPoint;                                     // The points number of region
    CFG_POLYGON             stuDetectRegion[MAX_POLYGON_NUM];                       // Detect region, point range[0,8192)
    BYTE                    byReserved[4096];                                       // Reversed
}CFG_WALK_AROUND_DETECT_INFO;

// Rule type EVENT_IVS_PLAY_MOBILEPHONE(Play mobile phone) configuration
typedef struct tagCFG_PLAY_MOBILEPHONE_INFO
{
    ...
    UINT                    nMinDuration;                                           // Minimum duration in seconds, range (10S?600S)
    UINT                    nReportInterval;                                        // Repeat alarm interval, unit: second, range (0s-300s)
    UINT                    nDetectRegionPoint;                                     // The points number of region
    CFG_POLYGON             stuDetectRegion[MAX_POLYGON_NUM];                       // Detect region, point range[0,8192)
    BYTE                    byReserved[4092];                                       // Reversed
...
// Rule type EVENT_IVS_FINANCE_CONTRABAND_DETECT(Detection of financial contraband) configuration
typedef struct tagCFG_FINANCE_CONTRABAND_DETECT_INFO
{
    ...
    UINT                    nMinDuration;                                           // Minimum duration in seconds, range (5S?0S)
    UINT                    nReportInterval;                                        // Repeat alarm interval, unit: second, range (0s-300s)
    UINT                    nDetectRegionPoint;                                     // The points number of region
    CFG_POLYGON             stuDetectRegion[MAX_POLYGON_NUM];                       // Detect region, point range[0,8192)
    UINT                    nHumanDetectRegionNum;                                  // The points number of human region
    CFG_POLYGON             stuHumanDetectRegion[MAX_POLYGON_NUM];                  // Detect human region, point range[0,8192)
    BYTE                    byReserved[4096];                                       // Reversed
}CFG_FINANCE_CONTRABAND_DETECT_INFO;

// Rule type EVENT_IVS_FINANCE_CASH_TRANSACTION(Financial cash transaction detection) configuration
...
// Rule type EVENT_IVS_ACTIVITY_ANALYSE configuration
typedef struct tagCFG_ACTIVITY_ANALYSE_INFO
{
    ...
    int                     nDetectRegionNum;                                       // The point number of region
    CFG_POLYLINE            stuDetectRegion[MAX_POLYGON_NUM];                       // Detect region, point range[0,8192)
	BYTE                    byReserved[4096];                                       // Reserved byte
} CFG_ACTIVITY_ANALYSE_INFO;

// Rule type EVENT_IVS_TRAFFIC_MOTOR_OVERLOAD configuration
typedef struct tagCFG_TRAFFIC_MOTOR_OVERLOAD_INFO
{
	...
	int                     nLaneNumber;											// Lane number
	int						nSnapTruckByPlateColor;									// Snap truck by plate color
																					// 0-snap yellow-plate truck only; 1-snap all；
	int						nSnapNoPlateMotor;										// Whether to snap no-plate motor；
																					// 0-not to snap; 1-snap
	int						nTruckNum;												// The number of truck type to detect
	EM_CFG_CATEGORY_TYPE	emTruckTypes[32];										// The list of truck type to detect
	BYTE                    byReserved[4096];                                       // Reserved byte
} CFG_TRAFFIC_MOTOR_OVERLOAD_INFO;

// Rule type EVENT_IVS_TRAFFIC_PLATE_OCCLUSION configuration
typedef struct tagCFG_TRAFFIC_PLATE_OCCLUSION_INFO
{
	...
	int                     nLaneNumber;											// Lane number
	BOOL					bSnapMotorcycle;										// Whether to snap motorcycle
	BYTE                    byReserved[4096];                                       // Reserved byte
...
typedef struct tagVIOLATIONCODE_INFO
{
    ...
    int                 nBigCarOverSpeedConfigNum;                                  // Big car over speed config info number
    CFG_BIG_CAR_OVERSPEED_INFO    stBigCarOverSpeedConfig[5];                       // Big car over speed config info
...
{
	BOOL				bSupportPan;					// Whether to support PTZ horizontal swing
	BOOL				bSupportTile;					// Whether to support PTZ vertical swing
	BOOL				bSupportZoom;					// Whether to support PTZ changed times 
...
{
	BOOL				bSupportNormal;					// Whether to support normalized move
	BOOL				bSupportReal;					// Whether to support unnormalized move
	BYTE				byReserved[120];				// Reserved
}CFG_PTZ_ABSOLUTELY_CAPS;

// The caps of ptz move absolutely 
typedef struct tagCFG_PTZ_MOVE_ABSOLUTELY_CAP
{
	CFG_PTZ_ACTION_CAPS stuPTZ;							// Ptz action types supported
	CFG_PTZ_ABSOLUTELY_CAPS stuType;					// Ptz absolutely types supported
...
typedef struct tagCFG_PTZ_PROTOCOL_CAPS_INFO
{
	...
	CFG_PTZ_MOVE_ABSOLUTELY_CAP	stuMoveAbsolutely;	// The caps of ptz move absolutely , Valid when bMoveAbsolutely==TRUE
...
typedef enum tagCFG_DOOR_OPEN_METHOD
{
	...
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_CARD_OR_FINGERPRINT_OR_FACE_OR_PASSWORD = 43,	// face card id or card or fingerprint or face or password
	CFG_DOOR_OPEN_METHOD_MULTI_USER_TYPE				= 44,	// multi user type
	CFG_DOOR_OPEN_METHOD_FACEIDCARD_OR_HEALTHCODE		= 45,	// card id and face or healthcode
...
#define CFG_MAX_ABLOCK_DOORS_NUM        8                       // Max access control door lock channel number
...
{
   EM_ALARMOUT_POLE_UNKNOWN,                                        // unknown
   EM_ALARMOUT_POLE_LOW,                                            // low level active
   EM_ALARMOUT_POLE_HIGH,                                           // high level active
} EM_ALARMOUT_POLE;
...
typedef struct tagCFG_ALARMOUT_INFO		// =>CFG_CMD_ALARMOUT
{
    ...
    EM_ALARMOUT_POLE    emPole;                                     // output valid mode
}CFG_ALARMOUT_INFO;
...
typedef struct tagCFG_CAP_EVENTMANAGER_INFO
{
	...
	DWORD				nSupportDisableLinkage;							  // Support disable linkage
                                                                          ...
                                                                          // bit5 means support disarming sound output
                                                                          // bit6 means support disarm white light output
    DWORD				nSupportDisableLinkageTimeSection;                // support disable linkage time section
	                                                                      // bit0 means support disable linkage time section
...
typedef struct tagCFG_IDLE_MOTION_INFO
{
	...
	int							nSecond;		 // enable idle action time(second) 0~59 second, total time is nTime * 60 + nSecond
...
typedef struct tagCFG_VTO_INFO 
{
    ...
    BYTE            byReserved[4];                              // Reserved bytes for byte alignment
    BOOL            bUseEx;                                     // Whether the extension of user name  and password is valid
    char            szLoginUsernameEx[CFG_COMMON_STRING_128];   // Extension of user name .The device currently supports a maximum 32-bit length and does not contain '\ 0'. The change is for compatibility with the device
    char            szLoginPassWordEx[CFG_COMMON_STRING_128];   // Extension of password.The device currently supports a maximum 32-bit length and does not contain '\ 0'. The change is for compatibility with the device
}CFG_VTO_INFO;
...
typedef enum tagEM_CFG_LC_MODE
{
    ...
    EM_CFG_LC_MODE_FORCEON,                             //Force light in
}EM_CFG_LC_MODE;
...
{
	EM_CFG_LC_LIGHT_TYPE	emLightType;					//Light Tyoe
	EM_CFG_LC_MODE			emMode;							//Light Mode
	int					nCorrection;					//Correction
	int					nSensitive;						//Sensitive
	int					nLightSwitchDelay;				//Switch delay
	NET_LIGHT_INFO		anNearLight[CFG_LC_LIGHT_COUNT];	//Near Light
	int					nNearLightLen;					//Number of near light
	NET_LIGHT_INFO		anMiddleLight[CFG_LC_LIGHT_COUNT];	//Middle Light
	int					nMiddleLightLen;				//Number of mid light
	NET_LIGHT_INFO		anFarLight[CFG_LC_LIGHT_COUNT];		//Far Light
...
{   
    DWORD                                   dwSize;                             
    BOOL                                    bAllMonitorWall;                    // Whether it is the setting of the whole monitor wall (valid only when setting), TRUE means to set all monitor walls(channel is -1); FALSE means to process the specified monitor wall(channel is not -1). 
    BYTE                                    byReserved[4];                      // Reserved bytes for byte alignment
...
// door not close configuration(corresponding to CFG_CMD_DOOR_NOT_CLOSE)
typedef struct tagCFG_DOOR_NOT_CLOSE_INFO
{
	BOOL							bEnable;				// Enable,true means on;false means off
	CFG_ALARM_MSG_HANDLE			stuEventHandler;		// Alarm activation      
}CFG_DOOR_NOT_CLOSE_INFO;

// break in configuration(corresponding to CFG_CMD_BREAK_IN)
...
// antipassback configuration(corresponding to CFG_CMD_ANTI_PASSBACK)
...
// duress configuration(corresponding to CFG_CMD_DURESS)
typedef struct tagCFG_DURESS_INFO
{
	BOOL							bEnable;				// Enable,true means on;false means off
	int								nDelayLinkTime;			// delay link time, unit:second, [1,300]
	CFG_ALARM_MSG_HANDLE			stuEventHandler;		// Alarm activation      
}CFG_DURESS_INFO;


// maliciousAccessControl configuration(corresponding to CFG_CMD_MALICIOUS_ACCESSCONTROL)
typedef struct tagCFG_DOOR_MALICIOUS_ACCESSCONTROL_INFO
{
	BOOL							bEnable;				// Enable,true means on;false means off
	int								nLimitedTimes;			// limit times, [2, 30]
	CFG_ALARM_MSG_HANDLE			stuEventHandler;		// Alarm activation      
}CFG_DOOR_MALICIOUS_ACCESSCONTROL_INFO;

// regulator detect configuration
